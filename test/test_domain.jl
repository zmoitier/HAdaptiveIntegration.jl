...
@testset "Domain construction" begin
    ...
    @testset "Segment" begin
        @test typeof(Segment(0, 1)) <: Segment{Int}
        @test typeof(Segment{Float64}(0, 1)) <: Segment{Float64}
...
@testset "Domain interface" begin
    ...
    @testset "Segment" begin
        ...
        @test abs_det_jac(s) ≈ 2

        @test typeof(reference_domain(Segment)) <: Segment{Float64}
        @test typeof(reference_domain(Segment{Int})) <: Segment{Int}
...
@testset "Domain Subdivision" begin
    @testset "segment" begin
        s = Segment(-0.81, 0.48)
        @test volume_check(subdivide_segment, s)
    end
