...
function main()
    ...
    for (col, hai_data, Iref, integrand, title, rtol) in (
        (1, hai_point, Iref_point, fct_point, "Point Feature", 1e-1),
        (2, hai_sphere, Iref_sphere, fct_sphere, "Hypersphere Feature", 4e-2),
        (3, hai_plane, Iref_plane, fct_plane, "Hyperplane Feature", 2e-2),
    )
    ...
    linkaxes!(axes_cvg...)
    xlims!(axes_cvg[1], 2e1, 4e7)
    ylims!(axes_cvg[1], 3e-12, 4e0)
