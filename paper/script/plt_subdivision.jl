...
function plot_subdivisions!(
    ax, domain, subdivide, draw!, offset, view_dir=nothing; colors=COLORS2
)
    b = barycenter(domain)
...
function plot_triangle!(fig, col)
    ...
    hidespines!(ax)
    plot_subdivisions!(
        ax, reference_domain(Triangle), subdivide_triangle, draw_triangle!, 0.1
    )
    return nothing
end
...
function plot_rectangle!(fig, col)
    ...
    hidespines!(ax)
    plot_subdivisions!(
        ax, reference_domain(Rectangle), subdivide_rectangle, draw_rectangle!, 0.1
    )
    return nothing
end
...
function draw_rectangle!(ax, rect, color)
    poly!(
        ax,
        collect(rectangle_vertices(rect));
        color=color,
        strokecolor=EDGE_COLOR,
        strokewidth=1,
    )
