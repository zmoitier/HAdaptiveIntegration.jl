...
Adaptive numerical integration is a fundamental building block in scientific computing, including finite-element, boundary-element methods, and in parameter studies over integrands with localized features.
...
When computing many integrals of the same type, the heap can be pre-allocated and passed via the `buffer` keyword so that, provided the integrand does not allocate; see the documentation for a zero-allocation benchmark.
