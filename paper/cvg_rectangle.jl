...
for (col, hai, hc, Iref, fct, title) in (
    ...
    p5 = lines!(ax, hai.N, e_ref .* (hai.N ./ N_ref) .^ (-high / 2); color = :black, linestyle = :dot, linewidth = 2)
    p6 = lines!(ax, hai.N, est_ref .* (hai.N ./ N_ref) .^ (-low / 2); color = :gray, linestyle = :dot, linewidth = 2)
